...
public class VendorFunctionTest extends UnitContainerTestCase {
    ...
    public void test_sepcify_derivedReferrer_option_coalesce_basic_Tx() throws Exception {
        ...
        }, Member.ALIAS_latestLoginDatetime, op -> op.coalesce("1900-01-01"));
        ...
        for (Member member : memberList) {
            ...
            if ("1900-01-01".equals(loginDateView)) {
                exists = true;
            }
        }
        assertTrue(exists);
    ...
    public void test_sepcify_derivedReferrer_option_trunc_basic_Tx() throws Exception {
        ...
        assertTrue(cb.toDisplaySql().contains("round("));
        assertTrue(cb.toDisplaySql().contains("), 0, 1)"));
    ...
    public void test_ColumnQuery_truncMonth_right() {
        ...
        assertTrue(Srl.contains(sql, " <> cast(substring(convert(nvarchar, dfloc.FORMALIZED_DATETIME"));
        assertTrue(Srl.contains(sql, ", dfloc.FORMALIZED_DATETIME, 120), 1, 4) + '-01-01' as datetime)"));
    }
    ...
    public void test_ColumnQuery_truncDay_right() {
        ...
        String sql = cb.toDisplaySql();
        assertTrue(Srl.contains(sql, " <> cast(substring(convert(nvarchar, dfloc.FORMALIZED_DATETIME"));
        assertTrue(Srl.contains(sql, ", dfloc.FORMALIZED_DATETIME, 120), 1, 7) + '-01' as datetime)"));
    }
    ...
    public void test_ColumnQuery_truncTime_right() {
        ...
        String sql = cb.toDisplaySql();
        assertTrue(Srl.contains(sql, " <> cast(substring(convert(nvarchar, dfloc.FORMALIZED_DATETIME"));
        assertTrue(Srl.contains(sql, ", dfloc.FORMALIZED_DATETIME, 120), 1, 10) as datetime)"));
    }
    ...
    public void test_ColumnQuery_dateAdd_collaboration() {
        ...
        String sql = cb.toDisplaySql();
        log(ln() + sql);
        assertTrue(Srl.contains(sql, "(cast(substring(convert(nvarchar, dateadd(day, 7, "));
        assertTrue(Srl.contains(sql, "(dfloc.FORMALIZED_DATETIME + 123)), 120), 1, 10) as datetime)) - 789"));
    }
    ...
    public void test_ColumnQuery_dateAdd_derived() {
        ...
        // ## Assert ##
        String sql = cb.toDisplaySql();
        assertTrue(Srl.contains(sql, "cast(substring(convert(nvarchar, dateadd(day, 3"));
        assertTrue(Srl.contains(sql, "dateadd(day, 3, (select max(sub2loc.PURCHASE_DATETIME)"));
    }
